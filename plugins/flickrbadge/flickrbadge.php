...
function flickrbadge($params=array()) {
  ...
  if(!$user || !$url) {
    ...
    if($options['cache']) cache::set($userCacheID, array(
      'user' => $user,
      'url'  => $url
    ));
  ...
  foreach($photos['photos']['photo'] as $photo) {
    ...
    if(empty($info)) {
      ...
      if($options['cache']) cache::set($photoCacheID, $info);
    }
    ...
    foreach((array)$info['tags']['tag'] as $t) {
      if(!empty($t['raw']) && !$t['machine_tag']) $tags[] = $t['raw'];
    }
            
    $result[] = new obj(array(
      'url'         => $url . $photo['id'],
      'title'       => a::get($info, 'title', $photo['title']),
      'description' => @$info['description'],
      'src'         => $flickr->buildPhotoURL($photo, $options['format']),
      'taken'       => isset($dates['taken'])  ? strtotime($dates['taken']) : false,
      'posted'      => isset($dates['posted']) ? $dates['posted'] : false,
      'lastupdate'  => isset($dates['lastupdate']) ? $dates['lastupdate'] : false,
      'views'       => a::get($info, 'views', 0),
      'comments'    => a::get($info, 'comments', 0),
      'tags'        => $tags
    ));
